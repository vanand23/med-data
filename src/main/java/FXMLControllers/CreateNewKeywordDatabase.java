...
public class CreateNewKeywordDatabase {
    ...
    //actually generates the new database (csv) file and can be viewed in the dropdown on the Add Keywords to Database page
    @FXML
    public void generateDatabase(ActionEvent actionEvent) throws IOException {
        ...
        if(!doesFileExist(newDatabaseNameString, "keywords")){ //check to see if the file exists
            File currDir = new File(".");
            String path = currDir.getAbsolutePath();

            //creates file path and adds the file to the Libraries/keywords directory
